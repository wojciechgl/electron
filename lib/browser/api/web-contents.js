...
WebContents.prototype.send = function (channel, ...args) {
  ...
  return this._send(false, false, channel, args)
}
WebContents.prototype.sendToAll = function (channel, ...args) {
  if (channel == null) throw new Error('Missing required channel argument')
  return this._send(false, true, channel, args)
}

WebContents.prototype._sendInternal = function (channel, ...args) {
  if (channel == null) throw new Error('Missing required channel argument')
  return this._send(true, false, channel, args)
}
WebContents.prototype._sendInternalToAll = function (channel, ...args) {
  if (channel == null) throw new Error('Missing required channel argument')
  return this._send(true, true, channel, args)
}
...
const syncWebFrameMethods = function (requestId, method, callback, ...args) {
  this._sendInternal('ELECTRON_INTERNAL_RENDERER_SYNC_WEB_FRAME_METHOD', requestId, method, args)
  ipcMainInternal.once(`ELECTRON_INTERNAL_BROWSER_SYNC_WEB_FRAME_RESPONSE_${requestId}`, function (event, result) {
