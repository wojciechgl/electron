...
const {isPromise, webContents} = electron
...
ipcMain.on('ELECTRON_BROWSER_SEND_TO', function (event, internal, sendToAll, webContentsId, channel, ...args) {
  ...
  if (internal) {
    if (sendToAll) {
      contents._sendInternalToAll(channel, ...args)
    } else {
      contents._sendInternal(channel, ...args)
    }
  } else {
    if (sendToAll) {
